...
const CandidateDocuments = () => {
  ...
  const filteredData = documentsData.filter(doc => {
    const matchesSearch = doc.documentName.toLowerCase().includes(filters.search.toLowerCase()) ||
                         doc.workOrderTitle.toLowerCase().includes(filters.search.toLowerCase());
    const matchesWorkOrder = filters.workOrder === 'all' || doc.workOrderId === filters.workOrder;
      ...
      render: (text, record) => (
        <Space direction="vertical" size="small" style={{ width: '100%' }}>
          ...
          <div className="mobile-only" style={{ display: 'none' }}>
            <Space wrap>
              <Tag color="#da2c46" size="small">{record.stage}</Tag>
        ...
        <Space size="small">
          <Tooltip title="Preview">
            <Button
              ...
              icon={<EyeOutlined />}
              size="small"
              onClick={() => setPreviewModal({ visible: true, document: record })}
              style={{ color: '#da2c46' }}
            />
          </Tooltip>
  ...
  const workOrderTimeline = selectedWorkOrder ? groupedData[selectedWorkOrder]?.documents.map(doc => ({
    children: (
      <div>
        ...
        <Col span={24}>
          <Card size="small">
            <Row align="middle" justify="space-between">
              <Col xs={24} sm={16} md={18}>
                <Space direction="vertical" size="small" style={{ width: '100%' }}>
                  <Space>
                    ...
                    />
                    <div>
                      <Title level={3} style={{ margin: 0, color: '#da2c46', fontSize: '18px' }}>
                        My Document History
                      </Title>
                      <Text type="secondary" style={{ fontSize: '12px' }}>
              ...
              <Col xs={24} sm={8} md={6} style={{ textAlign: 'center', marginTop: '8px' }}>
                <Badge 
        ...
        {Object.values(groupedData).map(workOrder => (
          <Col xs={24} sm={12} md={8} lg={6} xl={4} key={workOrder.workOrderId}>
            <Card
              hoverable
              size="small"
              onClick={() => setSelectedWorkOrder(
                selectedWorkOrder === workOrder.workOrderId ? null : workOrder.workOrderId
              )}
              style={{
                borderColor: selectedWorkOrder === workOrder.workOrderId ? '#da2c46' : undefined,
                borderWidth: selectedWorkOrder === workOrder.workOrderId ? '2px' : '1px',
                minHeight: '120px'
              }}
            >
              <div style={{ textAlign: 'center' }}>
                <Title level={5} style={{ color: '#da2c46', margin: '0 0 4px 0', fontSize: '14px' }}>
              ...
              expandable={{
                expandedRowRender: (record) => (
                  <div style={{ padding: '8px', backgroundColor: '#fafafa' }}>
                    <Row gutter={[8, 8]}>
                      <Col xs={24} sm={12}>
                        <Text strong style={{ fontSize: '12px' }}>Work Order: </Text>
                        <Text style={{ fontSize: '12px' }}>{record.workOrderTitle}</Text>
                      </Col>
                      <Col xs={24} sm={12}>
                        <Text strong style={{ fontSize: '12px' }}>Stage: </Text>
                        <Tag color="#da2c46" size="small">{record.stage}</Tag>
                      </Col>
                    </Row>
                  </div>
