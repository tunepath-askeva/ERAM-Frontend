...
const RecruiterDashboard = () => {
  ...
  // Only calculate totals if data exists
  const totalApplications =
    data?.applications?.byStatus?.reduce(
      (sum, item) => sum + (item.count || 0),
      0
    ) || 0;

  const totalInterviews =
    data?.interviews?.reduce((sum, item) => sum + (item.count || 0), 0) || 0;

  const hiredCount =
    data?.applications?.byStatus?.find((item) => item._id === "hired")?.count ||
    0;

  const completedInterviews =
    data?.interviews?.find((item) => item._id === "interview_completed")
      ?.count || 0;

  const totalCandidates = data?.candidates?.total || 0;

  // Prepare chart data for AntV - only if data exists
  ...
  const applicationColumns = [
    ...
    },
    {
      ...
      render: (record) => {
        const percentage =
          totalApplications > 0
            ? ((record.count / totalApplications) * 100).toFixed(1)
            : 0;
        return (
          <Progress
  ...
  const interviewColumns = [
    ...
    },
    {
      ...
      render: (record) => {
        const percentage =
          totalInterviews > 0
            ? ((record.count / totalInterviews) * 100).toFixed(1)
            : 0;
        return (
          <Progress
            percent={parseFloat(percentage)}
  ...
  if (!data) {
    return (
      <Alert
        ...
        description="No dashboard data found. Please check your connection or try again later."
        type="info"
        showIcon
        style={{ margin: "20px" }}
      />
  ...
  return (
    <div style={{ padding: "24px", minHeight: "100vh" }}>
      <div style={{ margin: "0 auto" }}>
        {/* Header with Get More Details Button */}
        <div
          style={{
            marginBottom: "24px",
            display: "flex",
        ...
        {/* Detailed Tables - Only render if data exists */}
        {(data?.applications?.byStatus || data?.interviews) && (
          <Row gutter={[24, 24]} style={{ marginBottom: "32px" }}>
