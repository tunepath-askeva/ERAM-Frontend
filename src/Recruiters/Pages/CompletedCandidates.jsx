...
import { useGetPipelineCompletedCandidatesQuery } from '../../Slices/Recruiter/RecruiterApis'; // Update this path
...
const CompletedCandidates = () => {
  ...
  const getFileIcon = (fileName) => {
    ...
    if (['jpg', 'jpeg', 'png', 'gif'].includes(extension)) return <FileImageOutlined style={{ color: '#1890ff' }} />;
  ...
  const renderCandidateDetails = () => {
    ...
    return (
      <div style={{ padding: '16px 0' }}>
        ...
        {/* Work Order Details */}
        <Card size="small" title="Work Order Details" style={{ marginBottom: '16px' }}>
          <Descriptions column={2} size="small">
            ...
            <Descriptions.Item label="Sourced">
              <Tag color={selectedCandidate.isSourced === 'true' ? 'success' : 'default'}>
        ...
        <Card size="small" title="Stages Progress">
          {selectedCandidate.stageProgress?.map((stage, index) => (
            <Card 
              ...
              style={{ marginBottom: index < selectedCandidate.stageProgress.length - 1 ? '12px' : 0 }}
            >
              {/* Recruiter Reviews */}
              {stage.recruiterReviews?.length > 0 && (
                <div style={{ marginBottom: '12px' }}>
                  ...
                  {stage.recruiterReviews.map((review, idx) => (
                    <div key={review._id} style={{ marginLeft: '16px', marginTop: '8px' }}>
              ...
              {stage.stageCompletedAt && (
                <Text type="secondary" style={{ fontSize: '12px' }}>
                  <ClockCircleOutlined /> Completed at: {formatDate(stage.stageCompletedAt)}
                </Text>
              )}
            </Card>
          ))}
        ...
        {selectedCandidate.interviewDetails?.length > 0 && (
          <Card size="small" title="Interview Details" style={{ marginTop: '16px' }}>
            {selectedCandidate.interviewDetails.map((interview, index) => (
              <Card key={interview._id} size="small" type="inner" style={{ marginBottom: '8px' }}>
                <Row gutter={16}>
                  <Col span={12}>
      ...
      stage.recruiterReviews?.forEach((review) => {
        if (review.reviewedAt) {
          timelineItems.push({
            ...
            children: (
              <div>
                <Text strong>{review.reviewerName} reviewed {stage.stageName}</Text>
                <br />
              ...
              <List.Item.Meta
                ...
                title={
                  <Space>
                    ...
                    <Tag size="small" color="blue">{document.stageName}</Tag>
                  </Space>
                }
                description={
                  <div>
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (stageProgress) => {
        const completed = stageProgress?.filter(stage => stage.stageStatus === 'approved').length || 0;
        const total = stageProgress?.length || 0;
        return (
          <div>
            <Text strong>{completed}/{total}</Text>
            <br />
            <Text type="secondary" style={{ fontSize: '12px' }}>stages</Text>
          </div>
        );
      },
    },
