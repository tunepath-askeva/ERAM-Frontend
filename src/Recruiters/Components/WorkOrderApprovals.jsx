...
const WorkOrderApprovals = ({
  ...
  const getTableData = () => {
    ...
    workOrders.forEach((workOrder) => {
      workOrder.pipelineStageTimeline.forEach((stage) => {
        // Skip separate approvals
        if (stage.separateApprovalId) return;
  ...
  return (
    <>
      ...
      {isMobile ? (
        <Drawer
          title={<Title level={5} style={{ margin: 0 }}>Document Review Details</Title>}
          placement="bottom"
        ...
        <Modal
          title={<Title level={4} style={{ margin: 0 }}>Document Review Details</Title>}
          visible={detailsModalVisible}
