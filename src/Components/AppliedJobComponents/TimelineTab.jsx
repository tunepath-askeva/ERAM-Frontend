...
const TimelineTab = ({ appliedJob }) => {
  ...
  const stageProgress = appliedJob?.stageProgress || [];
  ...
  return (
    <Card>
      ...
      <Timeline>
        ...
        {appliedJob?.status && (
          ...
          >
            <div>
              ...
              <Badge
                color={getStatusInfo(appliedJob.status).color}
                text={appliedJob.status?.replace("_", " ").toUpperCase() || "PENDING"}
              />
              {appliedJob.updatedAt && (
                <div style={{ marginTop: "4px" }}>
                  <Text type="secondary" style={{ fontSize: "12px" }}>
                    Last Updated: {new Date(appliedJob.updatedAt).toLocaleString()}
                  </Text>
                </div>
              )}
            </div>
        ...
        {appliedJob?.statusHistory?.length > 0 && (
          <Timeline.Item color="gray">
            <div>
              <Text strong>Status History</Text>
              <div style={{ marginTop: "8px" }}>
                {appliedJob.statusHistory.map((history, index) => (
