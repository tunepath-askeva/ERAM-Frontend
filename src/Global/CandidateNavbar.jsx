...
const CandidateNavbar = ({ collapsed, setCollapsed, setDrawerVisible }) => {
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      ...
      const emailToJoin = candidateData.email.toLowerCase();
      console.log("🚀 Emitting join with email:", emailToJoin); // DEBUG
    ...
    return () => {
      console.log("🔌 Disconnecting socket listeners"); // DEBUG
      socket.off("connect");
