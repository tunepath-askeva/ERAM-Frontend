...
const EmployeeNavbar = ({ collapsed, setCollapsed, setDrawerVisible }) => {
  ...
  useEffect(() => {
    const employeeDataRaw =
      localStorage.getItem("employeeInfo") || localStorage.getItem("employee");

    const employeeData = employeeDataRaw ? JSON.parse(employeeDataRaw) : null;

    if (!employeeData?.email) return;

    socket.on("connect", () => {
      console.log("Socket connected:", socket.id);
      const emailToJoin = employeeData?.email.toLowerCase();
  ...
  useEffect(() => {
    const fetchEmployeeInfo = () => {
      try {
        const possibleKeys = ["employeeInfo", "employee"];
